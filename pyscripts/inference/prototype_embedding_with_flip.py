...
#      embedding_with_location)
  ...
  def _compute_prototypes(image_batch_np, label_batch_np):
    ...
    (batch_prototype_features_np,
     batch_prototype_labels_np) = sess.run(
      ...
      feed_dict=feed_dict)
    return batch_prototype_features_np, batch_prototype_labels_np
  ...
  for step in pbar:
    ...
    (batch_prototype_features_np,
     batch_prototype_labels_np) = _compute_prototypes(
        image_batch_np, label_batch_np)
    ...
    if args.flip_aug:
      image_batch_np = image_batch_np[:, :, ::-1, :]
      label_batch_np = label_batch_np[:, :, ::-1, :]
